...
- Overlay finder component: `src/components/OverlayFinder.tsx`
...
- Search backend: `src/lib/db/notes/search.ts`
- FinderSearchPalette: `src/components/palette/FinderSearchPalette.tsx`
...
- Jump palette (`isJumpPaletteOpen`) and search palette (`isSearchPaletteOpen`) each gate the handler — return early if either is open.
- Input/textarea targets also early-return to avoid swallowing typing.
- `Escape` was added as a key to cancel a pending cut; it does NOT currently close the jump/search palette (those have their own handlers via PaletteModal).
...
- `/` key opens the search palette. Added as an early-return `if` block before the switch statement.

## Search Feature (implemented)

- Key: `/` opens `FinderSearchPalette` in both `finder.tsx` and `OverlayFinder.tsx`.
- Component: `src/components/palette/FinderSearchPalette.tsx`.
  - Uses `PaletteModal` wrapper (Portal + Escape + click-outside).
  - `createAsync` + `searchNotesWithDisplayTitlesQuery` for FTS5 search.
  - 200ms debounce on raw input before firing the server query via `onCleanup` + `setTimeout` in a `createEffect`.
  - Arrow keys / Ctrl+j/k navigate; Enter opens the note via `onSelectNote` callback.
  - `onSelectNote(noteId)` is called in both finders → wired to `openNote(noteId)`.
  - `isSearchPaletteOpen` signal guards the main keyboard handler (early return when open).
  - `requestAnimationFrame(() => inputRef?.focus())` for auto-focus on open.
  - `ContentItemData.path` field used for subtitle (may be undefined — guarded by `<Show when={item.path}>`).
  - `onMouseMove` on result rows keeps `selectedIndex` in sync with mouse position.
- `KeyboardHints` shows `/` search hint (added after the `z` jump palette hint).
