...
interface TiptapNoteEditorProps {
  ...
  placeholder?: string;
  /** Upload a file and return its URL. Used for drag-and-drop / paste. */
  onFileUpload?: (file: File) => Promise<{ url: string; name: string } | null>;
}
...
export default function TiptapNoteEditor(props: TiptapNoteEditorProps) {
  ...
  const editor = createTiptapEditor(() => ({
    ...
    editorProps: {
      ...
      handleDrop: (view, event, _slice, moved) => {
        if (moved || !props.onFileUpload) return false;
        const file = event.dataTransfer?.files?.[0];
        if (!file) return false;
        event.preventDefault();
        const pos = view.posAtCoords({ left: event.clientX, top: event.clientY })?.pos;
        props.onFileUpload(file).then((result) => {
          if (!result) return;
      ...
      handlePaste: (view, event) => {
        if (!props.onFileUpload) return false;
  ...
  return (
    <div class="flex-1 flex flex-col min-h-0">
      <Show when={editor()}>
        {(ed) => <Toolbar editor={ed()} onFileUpload={props.onFileUpload} />}
      </Show>
  ...
  const addImage = () => {
    if (props.onFileUpload) {
      ...
      input.onchange = () => {
        ...
        props.onFileUpload!(file).then((result) => {
          if (result) {
            ed().chain().focus().setImage({ src: result.url, alt: result.name }).run();
          }
        });
      };
      input.click();
