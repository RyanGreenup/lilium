...
import { MarkdownHtml } from "~/utils/renderMarkdown";
...
export const MarkdownRenderer = (props: {
  ...
  // Resolve markdown content for client-side rendering
  createEffect(() => {
    const syntax = props.syntax?.() || "md";
    ...
    if (isClientRenderedSyntax(syntax)) {
      ...
      try {
        ...
        if (syntax === "md") {
          ...
          md = convertedMarkdown() || props.content();
        } else {
          md = props.content();
        }
    ...
    <CodeBlockEnhancer>
      <Switch>
        ...
        <Match when={isPassthroughSyntax(props.syntax?.() || "")}>
          <div
            class="prose prose-sm max-w-none dark:prose-invert"
            innerHTML={props.content() || "No content"}
          />
        </Match>
