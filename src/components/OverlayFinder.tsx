...
import { getChildrenQuery, getNoteFolderPathQuery, LIST_CHILDREN_KEY, moveItem, moveItems } from "~/lib/db/api";
...
import { FinderSearchPalette } from "~/components/palette/FinderSearchPalette";

export interface OverlayFinderProps {
  open: Accessor<boolean>;
  onClose: () => void;
  revealNoteId?: Accessor<string | null>;
...
export default function OverlayFinder(props: OverlayFinderProps) {
  ...
  const [isSearchPaletteOpen, setIsSearchPaletteOpen] = createSignal(false);
  ...
  // Root folder ID — null means the true filesystem root.
  // (Future: change this to implement virtual root / "cd" behavior.)
  const [rootFolderId, setRootFolderId] = createSignal<string | null>(null);
  ...
  const createNewTab = async () => {
    const rootItems = await getChildrenQuery(rootFolderId());
    const newTab: TabState = {
      label: "Home",
      columns: [
        { folderId: rootFolderId(), items: rootItems, focusedIndex: 0, title: "Home" },
      ],
      depth: 0,
      focusMemory: {},
      previewItems: null,
  ...
  const createTabRevealingNote = async (noteId: string) => {
    // Reuse an existing tab if it already has this note focused.
    for (let i = 0; i < tabs.length; i++) {
      const tab = tabs[i];
      if (!tab || tab.depth < 0 || tab.depth >= tab.columns.length) continue;
      const col = tab.columns[tab.depth];
      if (!col) continue;
      const focused = col.items[col.focusedIndex];
      if (focused?.id === noteId) {
        switchToTab(i);
        return;
      }
    }
    ...
    const { folderPath } = pathInfo;
    const newColumns: ColumnEntry[] = [];
    ...
    if (folderPath.length > 0) {
      focusIdx = rootItems.findIndex(
        (i) => i.type === "folder" && i.id === folderPath[0]!.id,
      );
    } else {
      focusIdx = rootItems.findIndex((i) => i.id === noteId);
    }
    ...
    memoryUpdates[memoryKey(null)] = focusIdx;

    // Intermediate folder columns
    for (let i = 0; i < folderPath.length; i++) {
      const folder = folderPath[i]!;
      const children = await getChildrenQuery(folder.id);
      let childFocusIdx = 0;

      if (i < folderPath.length - 1) {
        childFocusIdx = children.findIndex(
          (c) => c.type === "folder" && c.id === folderPath[i + 1]!.id,
        );
      } else {
        childFocusIdx = children.findIndex((c) => c.id === noteId);
      }
      ...
      newColumns.push({
        folderId: folder.id,
        items: children,
        focusedIndex: childFocusIdx,
        title: folder.title,
      });
      memoryUpdates[memoryKey(folder.id)] = childFocusIdx;
    }

    const targetDepth = newColumns.length - 1;
    const label = deriveTabLabel(newColumns, targetDepth);

    const newTab: TabState = {
      label,
      columns: newColumns,
      depth: targetDepth,
      focusMemory: memoryUpdates,
      previewItems: null,
    };
  ...
  // Reveal a specific note when requested (e.g. Ctrl+Shift+G).
  createEffect(
    on(
      () => props.revealNoteId?.() ?? null,
      async (noteId) => {
        if (!noteId) return;
        // If overlay hasn't initialized yet, do it now.
        if (!initialized()) {
          setInitialized(true);
          const rootItems = await getChildrenQuery(rootFolderId());
          batch(() => {
        ...
        await createTabRevealingNote(noteId);
