 ...
 * Returns LinkItem format with path as subtitle
 */
export async function searchNotesForLinks(searchQuery: string): Promise<LinkItem[]> {
  ...
  // Convert to LinkItem format with full path as subtitle
  return results.map((note) => {
    // Build full path including the note title
